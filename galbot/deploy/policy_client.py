"""ZMQ client for remote policy inference (numpy + pyzmq only).
...
try:
    from galbot.deploy.wire.obs_codec import obs_to_multipart, reset_to_multipart
    from galbot.deploy.wire.trace_io import (
        coerce_nonneg_int,
...
class PolicyClient:
    """Synchronous REQ client for the policy server."""
    ...
    def reset(self) -> None:
        """Tell the remote policy to clear its per-episode state."""
        try:
    ...
    def inference(self, obs: dict) -> np.ndarray:
        ...
        maybe_save_obs_rgb_pngs(self._img_save, obs, self._inference_seq, log_error=self._log_error)

        try:
            reply = self._roundtrip(parts)
        except zmq.Again:
            self._log_error("ZMQ send/recv timed out; recreating REQ socket and retrying once.")
            self._recreate_req_socket()
            try:
                reply = self._roundtrip(parts)
            except zmq.Again:
                raise RuntimeError(
                    "ZMQ timed out twice; increase policy_zmq_timeout_ms (0 = no limit) "
